...
export default function DashboardClient({ user, userBookings }: { user: any, userBookings: any[] }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 relative overflow-hidden flex flex-col items-center py-12 px-4 sm:px-6" dir="rtl">
      {/* Dynamic Background */}
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-orange-400/10 rounded-full blur-[100px] mix-blend-multiply pointer-events-none" />
      ...
      <div className="max-w-6xl w-full relative z-10">
        ...
        <motion.div 
          ...
          className="relative overflow-hidden bg-gradient-to-l from-orange-600 via-orange-500 to-amber-500 p-8 sm:p-10 rounded-[2rem] shadow-xl shadow-orange-500/20 mb-10 text-white flex flex-col md:flex-row justify-between items-center gap-6"
        >
          ...
          <div className="relative z-10 flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-orange-100 text-sm sm:text-base font-medium">لوحة التحكم الخاصة بمعارضك وحجوزاتك</p>
            </div>
          </div>
          ...
          <div className="relative z-10 flex gap-3">
             ...
             <Link href="/api/auth/signout">
               <Button variant="outline" className="bg-white text-orange-600 border-none hover:bg-orange-50 shadow-lg">
                 تسجيل الخروج
               </Button>
             </Link>
          </div>
        ...
        <div className="space-y-6">
          ...
          >
            ...
            <h2 className="text-2xl font-bold text-slate-800">حجوزاتك السابقة <span className="text-orange-500 font-sans text-xl">({userBookings.length})</span></h2>
          </motion.div>
          ...
          {userBookings.length === 0 ? (
            ...
            >
              ...
              <Link href="/">
                <Button className="bg-gradient-to-r from-orange-600 to-orange-500 hover:from-orange-700 hover:to-orange-600 rounded-xl px-8 h-12 shadow-lg shadow-orange-500/20">
                ...
                >
                  <div className="flex justify-between items-start mb-5">
                    ...
                    <span className={`text-xs px-3 py-1.5 rounded-lg font-bold flex items-center gap-1.5 ${
                      ...
                      <span className={`w-1.5 h-1.5 rounded-full ${booking.status === 'confirmed' ? 'bg-emerald-500' : 'bg-orange-500 animate-pulse'}`} />
                      {booking.status === 'confirmed' ? 'مؤكد' : 'معلق'}
                    </span>
                  </div>
                  
                  <h3 className="font-bold text-xl text-slate-800 mb-1 group-hover:text-orange-600 transition-colors">{booking.entityName}</h3>
                  ...
                  <div className="bg-slate-50 rounded-2xl p-4 mb-6 border border-slate-100 flex-grow">
                     ...
                     <div className="flex justify-between text-sm">
                       ...
                       <span className="font-bold text-orange-600 bg-orange-50 px-2 py-0.5 rounded-md">{booking.totalPrice.toLocaleString()} ر.س</span>
                     </div>
                  </div>
                  ...
                  <div className="mt-auto flex gap-2 w-full pt-2">
                    <a href={`/api/contract/download/${booking.id}`} target="_blank" className="flex-1 w-full relative">
                      ...
                      <div className="absolute -inset-0.5 bg-gradient-to-r from-orange-400 to-amber-300 rounded-xl blur opacity-0 group-hover:opacity-40 transition duration-500 pb-1"></div>
