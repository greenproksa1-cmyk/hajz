...
export async function generateContract(bookingData: ContractData): Promise<Uint8Array> {
  ...
  y = pageHeight - 45;
  drawText('GREEN PROJECTS EXHIBITIONS & CONFERENCES', pageWidth / 2, y, { font: fontBold, size: 16, color: rgb(1,1,1), align: 'center' });
  y -= 25;
  drawText('OFFICIAL BOOTH RESERVATION CONTRACT', pageWidth / 2, y, { size: 10, color: rgb(0.7, 0.8, 0.9), align: 'center' });
  ...
  const terms = [
    '1. The exhibitor agrees to rent the specified booth(s) for the duration of the exhibition managed by Green Projects.',
  ...
  drawText('Green Projects Admin', box2X + 60, y - 40, { size: 9 });
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    currentPage.drawText('Green Projects Exhibitions & Conferences', { x: margin, y: footerY, size: 8, font: fontRegular, color: rgb(0.5, 0.5, 0.5) });
