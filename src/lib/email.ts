...
export async function sendOTPEmail(email: string, otp: string, lang: string = 'ar'): Promise<boolean> {
  const subjectAr = 'رمز التحقق - منصة قرين بروجكتس للمعارض';
  const subjectEn = 'Verification Code - Green Projects Exhibitions';

  const htmlAr = `
    <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      <div style="background: #16a34a; color: white; padding: 20px; border-radius: 10px 10px 0 0; text-align: center;">
        <h1>قرين بروجكتس للمعارض والمؤتمرات</h1>
      </div>
      <div style="border: 1px solid #ddd; padding: 30px; border-radius: 0 0 10px 10px; text-align: center;">
        <h2>رمز التحقق</h2>
        <p style="font-size: 18px; color: #666;">استخدم الكود التالي لإكمال عملية حجز الجناح:</p>
        <div style="font-size: 36px; font-weight: bold; color: #16a34a; letter-spacing: 8px; margin: 20px 0;">${otp}</div>
  ...
  const htmlEn = `
    <div dir="ltr" style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      <div style="background: #16a34a; color: white; padding: 20px; border-radius: 10px 10px 0 0; text-align: center;">
        <h1>Green Projects Exhibitions & Conferences</h1>
      </div>
      <div style="border: 1px solid #ddd; padding: 30px; border-radius: 0 0 10px 10px; text-align: center;">
        <h2>Verification Code</h2>
        <p style="font-size: 18px; color: #666;">Use the following code to complete your booth booking:</p>
        <div style="font-size: 36px; font-weight: bold; color: #16a34a; letter-spacing: 8px; margin: 20px 0;">${otp}</div>
  ...
  if (process.env.SMTP_HOST && process.env.SMTP_USER && process.env.SMTP_PASS) {
    try {
      ...
      const mailOptions = {
        from: process.env.SMTP_FROM || '"Green Projects" <contact@gren-pro.com>',
        to: email,
...
export async function sendConfirmationEmail(email: string, bookingId: string, entityName: string, lang: string = 'ar'): Promise<boolean> {
  const subjectAr = 'تأكيد الحجز - قرين بروجكتس للمعارض والمؤتمرات';
  const subjectEn = 'Booking Confirmed - Green Projects Exhibitions';
  ...
  const htmlAr = `
    <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="border: 1px solid #ddd; padding: 30px; border-radius: 0 0 10px 10px;">
        <h2 style="text-align: center;">تم تأكيد حجزك بنجاح</h2>
        <p>عزيزي/عزيزتي <strong>${entityName}</strong>،</p>
        <p>يسعدنا إعلامكم بأنه تم تأكيد حجز جناحكم بنجاح عبر منصة قرين بروجكتس.</p>
        <p><strong>رقم الحجز:</strong> ${bookingId}</p>
        <p style="color: #666; font-size: 14px;">سيتم إرسال تفاصيل إضافية والتعليمات التنظيمية عبر البريد الإلكتروني.</p>
      </div>
    </div>
  `;
  ...
  const htmlEn = `
    <div dir="ltr" style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="border: 1px solid #ddd; padding: 30px; border-radius: 0 0 10px 10px;">
        ...
        <p>We are pleased to confirm your booth booking via Green Projects Platform.</p>
        <p><strong>Booking ID:</strong> ${bookingId}</p>
        <p style="color: #666; font-size: 14px;">Additional details and exhibitor guide will be sent via email.</p>
      </div>
    </div>
  `;
...
export async function sendPasswordResetEmail(email: string, resetLink: string, lang: string = 'ar'): Promise<boolean> {
  const subjectAr = 'استعادة كلمة المرور - منصة قرين بروجكتس';
  const subjectEn = 'Password Reset - Green Projects Platform';

  const htmlAr = `
    <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      <div style="background: #16a34a; color: white; padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
        <h1 style="margin: 0;">قرين بروجكتس للمعارض والمؤتمرات</h1>
      </div>
      <div style="border: 1px solid #ddd; padding: 40px; border-radius: 0 0 10px 10px; text-align: center; background: white;">
        ...
        <div style="margin: 35px 0;">
          <a href="${resetLink}" style="background: #16a34a; color: white; padding: 14px 30px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 16px;">تعيين كلمة مرور جديدة</a>
        </div>
  ...
  const htmlEn = `
    <div dir="ltr" style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      <div style="background: #16a34a; color: white; padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
        <h1 style="margin: 0;">Green Projects Exhibitions & Conferences</h1>
      </div>
      <div style="border: 1px solid #ddd; padding: 40px; border-radius: 0 0 10px 10px; text-align: center; background: white;">
        ...
        <div style="margin: 35px 0;">
          <a href="${resetLink}" style="background: #16a34a; color: white; padding: 14px 30px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 16px;">Set New Password</a>
        </div>
